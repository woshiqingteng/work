...
#define DHT11_DATA_PIN 115  /* gpio4_19, (4-1)*32+19-115 */
...
static int __init dht11_init(void)
{
	...
	/* 1. 获得硬件信息 */
	err = gpio_request(DHT11_DATA_PIN, "DHT11");
	if (err) {
		printk("failed to request dht11 gpio\n");
		return -EIO;
	}
	
	dht11_data_pin = gpio_to_desc(DHT11_DATA_PIN); //gpiod_get(&pdev->dev, NULL, GPIOD_OUT_HIGH);

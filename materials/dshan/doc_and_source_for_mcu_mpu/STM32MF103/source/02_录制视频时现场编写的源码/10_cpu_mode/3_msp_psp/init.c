...
void SystemInit(void)
{
	...
	/* text relocate */
	if (&Image$$ER_IROM1$$Base != &Load$$ER_IROM1$$Base)
		memcpy((void *)&Image$$ER_IROM1$$Base, (const void *)&Load$$ER_IROM1$$Base, (unsigned int)&Image$$ER_IROM1$$Length);
	
	/* data relocate */
	if (&Image$$RW_IRAM1$$Base != &Load$$RW_IRAM1$$Base)
		memcpy((void *)&Image$$RW_IRAM1$$Base, (const void *)&Load$$RW_IRAM1$$Base, (unsigned int)&Image$$RW_IRAM1$$Length);

...
static void rt_hw_timer_isr(int vector, void *param)
{
     ...
     */
    TimerCvalWrite(TimerCvalRead() + 8000000);
    
    //TimerTvalWrite(8000000);
    TimerCtlWrite(1);
}
...
{    
	gic_enable_irq(OS_TICK_INT_NUM);
...
int system_counter_init(void)
{
	int g_sysClock;
	imx6ull_system_countter_init();
	HalClockFreqWrite(8000000);
    g_sysClock = HalClockFreqRead();

    printf("rt_hw_timer_init  OS_CYCLE_PER_TICK = ", g_sysClock);

	request_irq(OS_TICK_INT_NUM, rt_hw_timer_isr, 0);

...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if (key == WKUP_PRES)
	  {
		  ...
		  HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1, DAC_ALIGN_12B_R, dacval);/* 输出增大200 */
	  }
	  else if (key == KEY1_PRES)
	  {
		  ...
		  HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1, DAC_ALIGN_12B_R, dacval); /* 输出减少200 */
	  }

	  if (t == 10 || key == KEY1_PRES || key == WKUP_PRES)        /* WKUP/KEY1按下了,或者定时时间到了 */
	  {
		  adcx = HAL_DAC_GetValue(&hdac, DAC_CHANNEL_1);  		  /* 读取前面设置DAC1_OUT1的值 */
		  ...
		  adcx = adc3_get_result_average(ADC_CHANNEL_1, 20);           /* 得到ADC3通道1的转换结果 */

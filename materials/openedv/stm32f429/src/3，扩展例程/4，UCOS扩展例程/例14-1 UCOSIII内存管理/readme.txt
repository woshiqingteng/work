实验器材:
	阿波罗STM32F429开发板
	
实验目的:
	学习使用UCOSIII自带的内存管理。
	
硬件资源:
	1,DS0(连接在PB1)，DS1(连接在PB0上)
	2,串口1(波特率:115200,PA9/PA10连接在板载USB转串口芯片CH340上面) 
	3,ALIENTEK 2.8/3.5/4.3/7寸LCD模块(包括MCU屏和RGB屏,都支持) 
	4,按键KEY0(PH3)/KEY1(PH2)/KEY_UP(PA0,也称之为WK_UP)
	
实验现象:
	下载程序到开发板中，程序中创建了两个存储区，Internal_RamMemp和External_RamMemp，一个在
	STM32的内部ram中，一个在开发板上的SDRAM中。KEY_UP和KEY1对应着从Internal_RamMemp中申请和
	释放内存。KEY2和KEY0对应着从External_RamMemp中申请和释放内存。

	
注意事项:
	1,4.3寸和7寸屏需要比较大电流,USB供电可能不足,请用外部电源适配器(推荐外接12V 1A电源).
	2,本例程在LCD_Init函数里面(在lcd.c),用到了printf,如果不初始化串口1,将导致液晶无法显示!!  

	 

					正点原子@ALIENTEK
					2016-6-26
					广州市星翼电子科技有限公司
					电话：020-38271790
					传真：020-36773971
					购买：http://shop62103354.taobao.com
					http://shop62057469.taobao.com
					公司网站：www.alientek.com
					技术论坛：www.openedv.com

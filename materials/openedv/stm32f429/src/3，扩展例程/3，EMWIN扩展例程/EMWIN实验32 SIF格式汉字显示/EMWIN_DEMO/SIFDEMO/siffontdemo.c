...
static void _cbDialog(WM_MESSAGE * pMsg) 
{
	...
	switch (pMsg->MsgId) 
	{
		case WM_INIT_DIALOG:
			...
			FRAMEWIN_SetFont(hItem,&SIF16_Font);
			FRAMEWIN_SetTextColor(hItem, GUI_RED);
			FRAMEWIN_SetText(hItem, "ALIENTEK SIF格式汉字显示");
			...
			CHECKBOX_SetFont(hItem,&SIF16_Font);
			...
			RADIO_SetFont(hItem,&SIF16_Font);
			...
			DROPDOWN_SetFont(hItem,&SIF16_Font);
			...
			TEXT_SetFont(hItem,&SIF24_Font);
			TEXT_SetTextColor(hItem,GUI_TRANSPARENT);
			TEXT_SetText(hItem, "广州星翼电子");
			hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_3);
			TEXT_SetFont(hItem,&SIF36_Font);

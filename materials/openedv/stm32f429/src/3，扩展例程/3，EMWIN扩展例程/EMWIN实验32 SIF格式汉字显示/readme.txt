...
实验现象:	
	...
	汉字的显示使用SIF格式，SIF格式汉字字库保存在SD卡中。不同于XBF格式汉字的是，开发板
	每次上电以后首先将SIF格式汉字字库从SD卡中加载到SDRAM上，以后显示汉字的时候直接从
	SDRAM中读取汉字字库信息，因此SIF格式汉字的显示速度要比XBF快得多，但是也需要更多的RAM。

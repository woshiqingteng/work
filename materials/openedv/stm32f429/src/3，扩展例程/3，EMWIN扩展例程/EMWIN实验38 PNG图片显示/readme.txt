...
实验现象:	
	emWin/STemWin支持JPEG图片显示，本章例程就使用相关API函数实现了PNG图片在LCD上的显示，PNG图片
	保存在SD卡或者其他外部存储设备上。然后调用相关函数实现PNG图片的显示，缩放，指定位置显示等。
	PNG需要下载pnglib库，emWin/STemWin默认不带pnglib库。

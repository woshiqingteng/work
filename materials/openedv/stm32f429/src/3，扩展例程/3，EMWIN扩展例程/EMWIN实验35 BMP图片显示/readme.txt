...
实验现象:	
	emWin/STemWin支持BMP图片显示，本章例程就使用相关API函数实现了BMP图片在LCD上的显示，MBP图片
	保存在SD卡或者其他外部存储设备上。然后调用相关函数实现BMP图片的显示，缩放，指定位置显示等
	功能。同时本例程也支持屏幕截图，截图保存为BMP格式。

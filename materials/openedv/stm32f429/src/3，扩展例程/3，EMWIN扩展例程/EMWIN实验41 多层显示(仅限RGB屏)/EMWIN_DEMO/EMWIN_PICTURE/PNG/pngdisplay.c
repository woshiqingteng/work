...
int displaypng(char *PNGFileName,u8 mode,u32 x,u32 y)
{
	...
	if((result != FR_OK) || (PNGFile.obj.objsize>PNGMEMORYSIZE)) 	return 1;
	...
	{
		case 0:	//在指定位置显示图片
			GUI_PNG_Draw(pngbuffer,PNGFile.obj.objsize,x,y);//显示PNG图片
			break;
		case 1:	//在LCD中间显示图片
			GUI_PNG_Draw(pngbuffer,PNGFile.obj.objsize,(lcddev.width-XSize)/2-1,(lcddev.height-YSize)/2-1);	
			break;
	}

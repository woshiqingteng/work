...
实验现象:	
	emWin/STemWin支持多层显示，但是需要MCU自带LCD控制器，虽然最新的emWin已经支持软件多层，但是
	考虑到效率的问题，不建议使用软件多层。因此本例程只适用于RGB接口的屏幕，通过多层可以实现很多
	有意思的功能，本章例程就使用多层来实现了一个照片浏览器的APP。

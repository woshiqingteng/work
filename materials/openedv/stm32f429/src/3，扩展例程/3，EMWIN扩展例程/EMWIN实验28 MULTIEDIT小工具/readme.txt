...
实验目的:
	学习emWin/STemWin MULTIEDIT控件的使用。
...
实验现象:	
	下载程序到开发板，程序默认创建了一个FRAMWIN窗口，在FRAMWIN窗口上创建了三个控件，分别为
	一个MULTIEDIT控件、一个EDIT控件和一个BUTTON控件。串口1接收到数据以后就显示到MULTIEDIT
	控件中，当按下SEND键就会通过串口1发送EDIT控件中的字符串。

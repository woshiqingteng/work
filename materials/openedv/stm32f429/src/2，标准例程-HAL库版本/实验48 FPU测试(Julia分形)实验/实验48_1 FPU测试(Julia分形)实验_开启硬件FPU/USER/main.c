...
#include "sdram.h"
/************************************************
 ALIENTEK 阿波罗STM32F429开发板实验48_1
 FPU测试(Julia分形)实验 -HAL库函数版
...
void GenerateJulia_fpu(u16 size_x,u16 size_y,u16 offset_x,u16 offset_y,u16 zoom)
{
	...
	for(y=0;y<size_y;y++)
	{
       	for(x=0;x<size_x;x++)
   		{
			...
			if(lcdltdc.pwidth!=0)lcdbuf[lcddev.width-x-1]=color_map[i];		//保存颜色值到lcdbuf
			else LCD->LCD_RAM=color_map[i];//绘制到屏幕 
       	}
		if(lcdltdc.pwidth!=0)LTDC_Color_Fill(0,y,lcddev.width-1,y,lcdbuf);	//DM2D填充 
...
int main(void)
{
    ...
    SDRAM_Init();                   //初始化SDRAM
    LCD_Init();                     //初始化LCD
    TIM3_Init(65535,9000-1);        //10Khz计数频率,最大计时6.5秒超出
    POINT_COLOR=RED; 

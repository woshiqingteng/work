...
{      
	...
	{	 
		DMA2_Stream3->M0AR=(u32)i2sbuf[i2splaybuf];//指向下一个buf
	}
	else 
	{   		
		DMA2_Stream3->M1AR=(u32)i2sbuf[i2splaybuf];//指向下一个buf
	} 
} 
//得到path路径下,目标文件的总个数
//path:路径		    
...
void video_play(void)
{
	...
	u8 key;				//键值		  
 	u32 temp;
	u32 *voffblktbl;	//视频文件off block索引表
	...
	vfileinfo=(FILINFO*)mymalloc(SRAMIN,sizeof(FILINFO));//为长文件缓存区分配内存
 	...
 	voffblktbl=mymalloc(SRAMIN,4*totavinum);			//申请4*totavinum个字节的内存,用于存放视频文件索引
 	while(vfileinfo==NULL||pname==NULL||voffblktbl==NULL)//内存分配出错
 	{	    
	...
	{	
		dir_sdi(&vdir,voffblktbl[curindex]);			//改变当前目录索引	   
        res=f_readdir(&vdir,vfileinfo);       			//读取目录下的一个文件
        if(res!=FR_OK||vfileinfo->fname[0]==0)break;	//错误了/到末尾了,退出
...
{   
	...
	{ 
		...
		if(res==0)
		{
			...
			TIM6_Init(avix.SecPerFrame/100-1,9000-1);//10Khz计数频率,加1是100us 
			...
			if(avix.SampleRate)							//有音频信息,才初始化
			{
				...
				SAIA_TX_DMA_Init(i2sbuf[1],i2sbuf[2],avix.AudioBufSize/2,1);//配置DMA
				sai_tx_callback=audio_i2s_dma_callback;	//回调函数指向I2S_DMA_Callback
				i2splaybuf=0;
				i2ssavebuf=0; 
				SAI_Play_Start(); //开启I2S播放 
			}
 			while(1)//播放循环
			{					
			...
			SAI_Play_Stop();	//关闭音频
            __HAL_TIM_DISABLE(&TIM6_Handler);//关闭定时器6;

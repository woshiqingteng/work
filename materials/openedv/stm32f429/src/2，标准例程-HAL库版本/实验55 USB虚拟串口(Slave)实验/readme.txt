...
实验现象:
	...
	在找到虚拟串口后，即可打开串口调试助手，实现同实验3一样的功能，即：STM32通过USB虚拟串口和上位
	机对话，STM32在收到上位机发过来的字符串（以回车换行结束）后，原原本本的返回给上位机。下载后，
	DS0闪烁，提示程序在运行，同时每隔一定时间，通过USB虚拟串口输出一段信息到电脑。 
...
参考资料：阿波罗STM32F429开发指南-库函数版本.pdf 第六十章

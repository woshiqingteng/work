...
void audio_play(void)
{
	...
	u8 key;				//键值		  
 	u32 temp;
	u32 *wavoffblktbl;	//音乐off block索引表
	...
	wavfileinfo=(FILINFO*)mymalloc(SRAMIN,sizeof(FILINFO));	//申请内存
 	...
 	wavoffblktbl=mymalloc(SRAMIN,4*totwavnum);				//申请4*totwavnum个字节的内存,用于存放音乐文件off block索引
 	while(!wavfileinfo||!pname||!wavoffblktbl)//内存分配出错
 	{	    
	...
	{	
		dir_sdi(&wavdir,wavoffblktbl[curindex]);				//改变当前目录索引	   
        res=f_readdir(&wavdir,wavfileinfo);       				//读取目录下的一个文件
        if(res!=FR_OK||wavfileinfo->fname[0]==0)break;			//错误了/到末尾了,退出		 
	...
	myfree(SRAMIN,wavoffblktbl);		//释放内存	 
} 
//播放某个音频文件

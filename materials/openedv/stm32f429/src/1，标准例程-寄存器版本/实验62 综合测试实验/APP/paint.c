...
{	 
	...
	else if(gui_phy.memdevflag&(1<<3))sprintf((char*)pname,"3:PAINT/PAINT%04d%02d%02d%02d%02d%02d.bmp",calendar.w_year,calendar.w_month,calendar.w_date,calendar.hour,calendar.min,calendar.sec);	//SD卡不存在,则保存在U盘	
}
...
u8 paint_play(void)
{
	...
	_filelistbox_list * filelistx=0;//文件表
 	u16 lastpos[CT_MAX_TOUCH][2];	//最后一次的数据 
		...
		{		 
			...
			if(tp_dev.touchtype&0X80)		//电容屏
			{
				for(res=0;res<CT_MAX_TOUCH; res++)
				{
					if((tp_dev.sta)&(1<<res))
					{
						if(tp_dev.x[res]<(lcddev.width-(mode&0x0f))&&tp_dev.y[res]<(lcddev.height-(mode&0x0f))) //限定坐标
						{
